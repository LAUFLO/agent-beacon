...
namespace AgentTrafficLightNative {
  ...
  sealed class HistoryForm : Form {
    ...
    public HistoryForm() {
      Text = "Agent Beacon 状态中心"; Icon = PixelTheme.AppIcon; ClientSize = new Size(760, 600); StartPosition = FormStartPosition.CenterParent; FormBorderStyle = FormBorderStyle.None; MaximizeBox = false; MinimizeBox = false;
      ...
      var title = PixelTheme.Label("AGENT BEACON // 状态中心", new Point(60, 8), new Size(610, 32), true); Controls.Add(title);
      ...
      diagnostic.Location = new Point(30, 88); diagnostic.Size = new Size(700, 102); diagnostic.BackColor = PixelTheme.Paper; diagnostic.ForeColor = PixelTheme.Ink; diagnostic.Font = PixelTheme.MonoFont; Controls.Add(diagnostic);
      Controls.Add(PixelTheme.Label("状态变化历史", new Point(24, 210), new Size(200, 24), true));
      Controls.Add(PixelTheme.Label("仅保存时间、Agent、颜色、说明与判定来源，不保存聊天正文。", new Point(180, 210), new Size(520, 24), false));
      content.Location = new Point(30, 240); content.Size = new Size(700, 272); content.BackColor = PixelTheme.Paper; content.ForeColor = PixelTheme.Ink; content.Font = PixelTheme.MonoFont; Controls.Add(content);
      var refresh = new PixelButton { Text = "刷新全部", Active = true, Location = new Point(24, 542), Size = new Size(108, 34) }; refresh.Click += delegate { Reload(); }; Controls.Add(refresh);
      var copyDiagnostic = new PixelButton { Text = "复制诊断", Location = new Point(148, 542), Size = new Size(108, 34) }; copyDiagnostic.Click += delegate { try { Clipboard.SetText(diagnostic.Text); copyDiagnostic.Text = "已复制"; } catch { copyDiagnostic.Text = "复制失败"; } }; Controls.Add(copyDiagnostic);
      var copyHistory = new PixelButton { Text = "复制历史", Location = new Point(272, 542), Size = new Size(108, 34) }; copyHistory.Click += delegate { try { Clipboard.SetText(content.Text); copyHistory.Text = "已复制"; } catch { copyHistory.Text = "复制失败"; } }; Controls.Add(copyHistory);
      var clear = new PixelButton { Text = "清空历史", Danger = true, Location = new Point(624, 542), Size = new Size(108, 34) }; clear.Click += delegate { if (PixelDialog.Show(this, "确定清空本机状态变化历史吗？\n\n实时诊断不会被清除。", "清空状态历史", PixelDialogButtons.YesNo) == DialogResult.Yes) { StateHistory.Clear(); Reload(); } }; Controls.Add(clear);
    ...
    protected override void OnPaint(PaintEventArgs e) { PixelTheme.PaintWindow(e.Graphics, Width, Height, 0); using (var ink = new Pen(PixelTheme.Ink, 4)) { e.Graphics.DrawRectangle(ink, 24, 82, 712, 114); e.Graphics.DrawRectangle(ink, 24, 234, 712, 284); } base.OnPaint(e); }
  }
}
