...
namespace AgentTrafficLightNative {
  ...
  sealed class SettingsForm : Form {
    ...
    public SettingsForm(SettingsData settings, Action<int> intervalChanged, Action<bool> modeChanged, Action<int> scaleChanged, Action updateCheck) {
      ...
      var title = new Label { Text = "AGENT BEACON // 设置", AutoSize = false, Location = new Point(60, 8), Size = new Size(610, 32), ForeColor = PixelTheme.Ink, BackColor = Color.Transparent, Font = PixelTheme.TitleFont, TextAlign = ContentAlignment.MiddleCenter }; Controls.Add(title);
      ...
      Controls.Add(new Label { Text = "刷新频率", AutoSize = false, Location = new Point(22, 126), Size = new Size(72, 26), TextAlign = ContentAlignment.MiddleCenter, ForeColor = PixelTheme.Muted, BackColor = Color.Transparent, Font = PixelTheme.StrongFont });
      ...
      var trae = new PixelButton { Text = "复制 TRAE MCP 配置", Location = new Point(22, 232), Size = new Size(238, 36) }; trae.Click += delegate { string message = Integration.InstallTraeMcp(); refresh(); PixelDialog.Show(this, message, "TRAE MCP", PixelDialogButtons.Ok); }; Controls.Add(trae);
      var rule = new PixelButton { Text = "复制 TRAE 状态规则", Location = new Point(22, 278), Size = new Size(238, 36) }; rule.Click += delegate { string message = Integration.CopyTraeRule(); PixelDialog.Show(this, message, "TRAE 状态规则", PixelDialogButtons.Ok); }; Controls.Add(rule);
      var claude = new PixelButton { Text = "安装 / 更新 CLAUDE HOOKS", Location = new Point(22, 324), Size = new Size(238, 36) }; claude.Click += delegate { string message = Integration.InstallClaude(); refresh(); PixelDialog.Show(this, message, "CLAUDE HOOKS", PixelDialogButtons.Ok); }; Controls.Add(claude);
      var openCode = new PixelButton { Text = "安装 / 更新 OPENCODE 插件", Location = new Point(22, 370), Size = new Size(238, 36) }; openCode.Click += delegate { string message = Integration.InstallOpenCode(); refresh(); PixelDialog.Show(this, message, "OPENCODE 插件", PixelDialogButtons.Ok); }; Controls.Add(openCode);
      ...
      var quiet = new PixelToggle { Text = "免打扰 22:00 - 08:00", Checked = settings.QuietHoursEnabled, Location = new Point(454, 273), Width = 280 }; quiet.CheckedChanged += delegate { settings.QuietHoursEnabled = quiet.Checked; Program.SaveSettings(settings); }; Controls.Add(quiet);
      ...
      var history = new PixelButton { Text = "打开状态中心", Location = new Point(454, 440), Size = new Size(238, 36) }; history.Click += delegate { using (var form = new HistoryForm()) form.ShowDialog(this); }; Controls.Add(history);
    ...
    Label StatusLabel(int x, int y) { return new Label { AutoSize = false, Location = new Point(x, y), Size = new Size(126, 24), BackColor = Color.Transparent, Font = PixelTheme.StrongFont, TextAlign = ContentAlignment.MiddleCenter }; }
    ...
    protected override void OnPaint(PaintEventArgs e) {
      ...
      using (var rule = new SolidBrush(PixelTheme.Ink)) { g.FillRectangle(rule, 14, 112, 402, 3); g.FillRectangle(rule, 14, 166, 402, 3); g.FillRectangle(rule, 14, 217, 402, 3); g.FillRectangle(rule, 14, 414, 402, 3); }
      base.OnPaint(e);
    }
  }
  ...
  sealed class MainForm : Form {
    ...
    public MainForm(SettingsData loaded) {
      ...
      Text = "Agent Beacon v" + AppInfo.Version; Name = "AgentBeaconWindow"; Icon = PixelTheme.AppIcon; Width = (int)Math.Round(38 * initialScale); Height = (int)Math.Round(88 * initialScale); BackColor = PixelPoleControl.KeyColor; TransparencyKey = PixelPoleControl.KeyColor; ForeColor = Color.White; FormBorderStyle = FormBorderStyle.None; ShowInTaskbar = Environment.GetEnvironmentVariable("AGENT_BEACON_UI_TEST") == "1"; StartPosition = FormStartPosition.Manual; Location = new Point(Screen.PrimaryScreen.WorkingArea.Right - Width - 20, Screen.PrimaryScreen.WorkingArea.Top + 20); TopMost = true; DoubleBuffered = true;
    ...
    void ProcessStateTransitions(List<AgentTask> agents) {
      ...
      foreach (var task in agents) {
        ...
        if (!existed || !String.Equals(previous, task.Status, StringComparison.OrdinalIgnoreCase)) {
          ...
          if (transitionBaselineReady && NotificationPolicy.ShouldNotify(settings, task)) ShowStateNotification(task);
        }
      }
