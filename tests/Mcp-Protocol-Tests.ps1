...
try {
  ...
  if ($initialize.result.serverInfo.version -ne '1.5.0') { throw 'TRAE MCP version is not 1.5.0.' }
  ...
  if (-not $health.connected -or $health.helperVersion -ne '1.5.0') { throw 'TRAE MCP live health handshake was not recorded.' }
