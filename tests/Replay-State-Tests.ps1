...
$codexPromptElement = $processType.GetMethod('ShouldConsiderCodexApprovalPromptElement', [Reflection.BindingFlags]'Static,NonPublic,Public')
$codexScanStart = $processType.GetMethod('CodexAutomationScanStart', [Reflection.BindingFlags]'Static,NonPublic,Public')
$codexUiScanDelay = $processType.GetMethod('CodexUiScanDelay', [Reflection.BindingFlags]'Static,NonPublic,Public')
if (-not $codexUiProbe -or -not $codexPromptText -or -not $codexActionText -or -not $codexApprovalElement -or -not $codexPromptElement -or -not $codexScanStart -or -not $codexUiScanDelay -or -not ($assembly.GetReferencedAssemblies().Name -match '^UIAutomation')) { throw 'Minimal Codex approval probe is missing.' }
if (-not $codexPromptText.Invoke($null, @('Do you want to allow ChatGPT to run this command?')) -or -not $codexPromptText.Invoke($null, @('Allow GitHub to create a pull request?')) -or -not $codexPromptText.Invoke($null, @('Approval required by Codex')) -or -not $codexPromptText.Invoke($null, @('是否允许补充当前用户 PATH，使安装后的 kdocs-cli 可在新终端中直接运行？')) -or -not $codexPromptText.Invoke($null, @('终端 是否允许将已验证的 guard-project-quality Skill 安装到你的全局 Codex Skills 目录？')) -or $codexPromptText.Invoke($null, @('GitHub can create a pull request.'))) { throw 'Codex approval prompt text was not recognized safely.' }
...
if (-not $codexPromptElement.Invoke($null, @($true, $false)) -or -not $codexPromptElement.Invoke($null, @($false, $false))) { throw 'Static Codex approval prompt text is incorrectly discarded when its accessibility node is disabled.' }
if ($codexScanStart.Invoke($null, @(750, 600)) -ne 150 -or $codexScanStart.Invoke($null, @(40, 200)) -ne 0) { throw 'Codex approval scanning does not prioritize the newest bounded accessibility elements.' }
...
if ($agentProcessesSource -match '(?:current|cached)\.IsOffscreen\)\s*continue' -or $agentProcessesSource -notmatch 'ShouldConsiderCodexApprovalElement\(cached\.IsOffscreen, cached\.IsEnabled\)' -or $agentProcessesSource -notmatch 'ShouldConsiderCodexApprovalPromptElement\(cached\.IsOffscreen, cached\.IsEnabled\)') { throw 'Codex UI probe still skips backgrounded approval prompts or actions.' }
...
if (-not $safeTarget.Invoke($null, @('D:\Agent-Beacon-1.6.3.exe')) -or $safeTarget.Invoke($null, @('C:\Windows\System32\notepad.exe'))) { throw 'Automatic update target validation failed.' }
...
if ($appInfoType.GetField('Version', [Reflection.BindingFlags]'Static,NonPublic,Public').GetRawConstantValue() -ne '1.6.3') { throw 'Application version metadata is not 1.6.3.' }
